...
async def create_auction(auction: AuctionCreate, current_user: UserResponse = Depends(auth.get_current_manager), db: Session = Depends(get_db)):
...
async def update_auction(auction_id: int, current_user: UserResponse = Depends(auth.get_current_manager), db: Session = Depends(get_db)):
...
async def add_item_to_auction(auction_id: int, item: AuctionItemCreate, current_user: UserResponse = Depends(auth.get_current_manager), db: Session = Depends(get_db)):
...
async def end_auction(auction_id: int, current_user: UserResponse = Depends(auth.get_current_manager), db: Session = Depends(get_db)):
